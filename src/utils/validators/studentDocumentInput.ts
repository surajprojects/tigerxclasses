...
export const studentDocumentInput = z.object({
    ...
    aadhaarNo: z.string().regex(/^\d{12}$/, "Aadhaar No. must be 12 digits number string. Example - '1234 5678 9012'").optional(),
    rollNo: z.string().regex(/^\d{1,12}$/, "Roll No. must be between 1 to 12 digits number string. Example - '123456789012'").optional(),
    enrollmentNo: z.string().optional(),
    obtainedMarks: z.string().regex(/^\d{1,8}$/, "Obtained Marks must be between 1 to 8 digits number string. Example - '12345678'").optional(),
    totalMarks: z.string().regex(/^\d{1,8}$/, "Total Marks must be between 1 to 8 digits number string. Example - '12345678'").optional(),
